...
public:
            ...
            int64_t p = 2, const std::vector<int64_t>& dilations = {}, bool ceil_mode = false)
    {
        ...
        for (size_t i = 0; i < spatial_dims; ++i)
        {
            ...
            if (ceil_mode) {
                // 使用向上取整 (ceil) 函数
                double temp = static_cast<double>(input_dim + pad_head + pad_tail - dilated_kernel_size) / stride + 1;
            ...
            for (int64_t c = 0; c < output_shape[1]; ++c)
            {
                ...
                while (!done)
                {
                    ...
                    if (count > 0)
                    {
                        /* 
                         * 严格按照 ONNX 实现的计算方式：
                         * 参考 https://github.com/onnx/onnx/blob/main/onnx/reference/ops/op_pool_common.py
                         *
                         * LpPool 实现的核心思路：
