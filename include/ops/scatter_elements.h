...
public:
    ...
    Compute(const std::vector<T>& data, const std::vector<int64_t>& data_shape,
            ...
            const std::vector<T>& updates, const std::vector<int64_t>& updates_shape,
            int64_t axis = 0, const std::string& reduction = "none")
    {
        ...
        {
            throw std::invalid_argument("indices and updates must have the same shape");
        }
        if (axis < 0)
