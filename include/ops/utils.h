...
inline int64_t coords_to_offset(const std::vector<int64_t>& coords,
...
/// 根据n维坐标获取值
template <typename T>
T get_value(const std::vector<int64_t>& coords, const std::vector<int64_t>& shape,
            std::vector<T>& data)
{
...
// 根据n维坐标设置值
template <typename T>
void set_value(const std::vector<int64_t>& coords, T value, std::vector<T>& data,
               const std::vector<int64_t>& shape)
{
