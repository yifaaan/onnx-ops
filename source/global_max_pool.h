...
template <typename T>
std::pair<std::vector<T>, std::vector<int64_t>> GlobalMaxPool(const std::vector<T>& data,
                                                              const std::vector<int64_t>& shape)
{
    ...
    for (int64_t n = 0; n < batch_size; ++n)
    {
        for (int64_t c = 0; c < channels; ++c)
        {
            ...
            // 计算当前批次和通道的基础偏移
            int64_t base_offset = n * channels * elements_per_channel + c * elements_per_channel;
