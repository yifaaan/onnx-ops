...
GatherElements(const std::vector<T>& data, const std::vector<int64_t>& data_shape,
               ...
               int64_t axis = 0)
{
    ...
    for (int d = 0; d < indices_shape.size(); d++)
    {
        if (d != axis && data_shape[d] != indices_shape[d])
        {
            throw std::invalid_argument(
                "indices_shape must be the same as data_shape except for the axis dimension");
        }
    }
    ...
    // 计算输出元素的个数
    auto len = std::accumulate(std::begin(indices_shape), std::end(indices_shape), 1,
