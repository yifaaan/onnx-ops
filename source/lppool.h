...
// 计算输出形状
inline std::vector<int64_t> compute_output_shape(const std::vector<int64_t>& shape,
                                                 ...
                                                 const std::string& auto_pad, bool ceil_mode)
{
    ...
    for (size_t i = 0; i < spatial_dims; ++i)
    {
        ...
        if (auto_pad == "SAME_UPPER" || auto_pad == "SAME_LOWER")
        {
            ...
            pad_head =
                auto_pad == "SAME_UPPER" ? padding_needed / 2 : padding_needed - padding_needed / 2;
        ...
        // 计算输出维度
        double temp =
            static_cast<double>(input_dim + pad_head + pad_tail - dilated_kernel_size) / stride + 1;
...
// 执行池化计算
template <typename T>
void compute_pooling(const std::vector<T>& data, std::vector<T>& output_data,
                     ...
                     int64_t p, const std::vector<int64_t>& input_strides,
    ...
    for (int64_t n = 0; n < batch_size; ++n)
    {
        for (int64_t c = 0; c < channels; ++c)
        {
            ...
            for (int out_idx = 0; out_idx < len; ++out_idx)
            {
                ...
                // 池化窗口的元素个数
                int kernel_num_size = std::accumulate(kernel_shape.begin(), kernel_shape.end(), 1,
...
LpPool(const std::vector<T>& data, const std::vector<int64_t>& shape,
       const std::vector<int64_t>& kernel_shape, const std::vector<int64_t>& strides = {},
       const std::vector<int64_t>& pads = {}, const std::string& auto_pad = "NOTSET", int64_t p = 2,
       const std::vector<int64_t>& dilations = {}, bool ceil_mode = false)
{
